...
function TopicForm({ setResult, setLoading, loading, setError }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const credits = error.response?.data?.creditsLeft;
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (value >= 95) {
        ...
        setProgressText("Finilizing Notes...");
      } else if (value > 40) {
  ...
  return (
    ...
    >
      ...
      <input
        ...
        text-white
        focus:outline-none focus:ring-2 focus:ring-white/30"
        placeholder="Exam Type (e.g. CBSE, JEE, Placement)"
      ...
      >
        {loading ? "Generating Notes..." : "Generated Notes"}
      </motion.button>
      {loading && (
        <div className="mt-4 space-y-2">
          <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
            <motion.div
              ...
              className="h-full bg-gradient-to-r from-green-300 via-green-400 to-green-500"
            ></motion.div>
          </div>
      ...
      <motion.div
        animate={{
          backgroundColor: checked
            ? "rgba(34, 197, 94, 0.35)" // Turn On
            : "rgba(255, 255, 255, 0.15)", // Turn Off
        }}
