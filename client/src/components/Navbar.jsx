          ...
          <AnimatePresence>
            {showCredits && (
              ...
              shadow-[0_25px_60px_rgba(0,0,0,0.7)
               p-4 text-white]"
              >
                <h4 className="font-semibold mb-2 text-white">Buy Credits</h4>
                <p className="text-sm text-gray-300 mb-4">
                  Use Credit to generate AI notes, diagrams & PDFs.
                </p>
                <button
                  onClick={() => {setShowCredits(false); navigate('/pricing')}}
                  className="w-full py-2 rounded-lg bg-gradient-to-br 
          ...
          <AnimatePresence>
            {showProfile && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10, scale: 0.95 }}
                transition={{ duration: 0.2 }}
                className="absolute right-[-50px] mt-4 w-50 
              rounded-2xl 
              bg-black/90 backdrop-blur-xl 
              border border-white/10 
              shadow-[0_25px_60px_rgba(0,0,0,0.7)
  ...
  return (
    <div
      ...
      className={`w-full text-left px-5 py-3 text-sm   
            transaction-color rounded-lg cursor-pointer ${
              red
